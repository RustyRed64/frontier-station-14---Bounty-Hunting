...
public sealed partial class BountyHunterSystem : EntitySystem
{
    ...
    //private List<BountyHunterPrototype> _cachedPrototypes = new();
    ...
    public override void Initialize()
    {
        ...
        //SubscribeLocalEvent<BountyHunterTargetComponent, ComponentStartup>(InitializeBountyHunterBounty);
        //SubscribeLocalEvent<BountyHunterTargetComponent, MobStateChangedEvent>(OnMobStateChanged);

        SubscribeLocalEvent<BountyHunterRedemptionComponent, RedeemBountyHunterMessage>(RedeemMedicalBounty);
        SubscribeLocalEvent<BountyHunterRedemptionComponent, EntInsertedIntoContainerMessage>(OnEntityInserted);
        SubscribeLocalEvent<BountyHunterRedemptionComponent, EntRemovedFromContainerMessage>(OnEntityRemoved);
        SubscribeLocalEvent<BountyHunterRedemptionComponent, AfterActivatableUIOpenEvent>(OnActivateUI);
        SubscribeLocalEvent<BountyHunterRedemptionComponent, PowerChangedEvent>(OnPowerChanged);
    ...
    // private void InitializeBountyHunterBounty(EntityUid entity, BountyHunterTargetComponent component, ComponentStartup args)
    // {
    ...
    private void RedeemMedicalBounty(EntityUid uid, BountyHunterRedemptionComponent component, RedeemBountyHunterMessage ev)
    {
        // Check that the bounty redeemer has a valid bounty inside
        if (!_container.TryGetContainer(uid, component.Bounty_Container, out var container) ||
  ...
  //      }
    ...
    private void OnEntityInserted(EntityUid uid, BountyHunterRedemptionComponent component, EntInsertedIntoContainerMessage args)
    {
        UpdateUserInterface(uid, component);
        _appearance.SetData(uid, BountyHunterRedemptionVisuals.Full, true);
    }

    private void OnEntityRemoved(EntityUid uid, BountyHunterRedemptionComponent component, EntRemovedFromContainerMessage args)
    {
        UpdateUserInterface(uid, component);
        _appearance.SetData(uid, BountyHunterRedemptionVisuals.Full, false);
    }

    private void OnActivateUI(EntityUid uid, BountyHunterRedemptionComponent component, AfterActivatableUIOpenEvent args)
    {
        UpdateUserInterface(uid, component);
    }

    private void OnPowerChanged(EntityUid uid, BountyHunterRedemptionComponent component, PowerChangedEvent args)
    {
        UpdateUserInterface(uid, component);
    }

    public void UpdateUserInterface(EntityUid uid, BountyHunterRedemptionComponent component)
    {
        if (!_ui.HasUi(uid, BountyHunterRedemptionUiKey.Key))
            return;

        var actor = _ui.GetActors(uid, BountyHunterRedemptionUiKey.Key).FirstOrDefault();
        ...
        _ui.SetUiState(uid, BountyHunterRedemptionUiKey.Key, GetUserInterfaceState(uid, component, actor));
    }
    ...
    private BountyHunterRedemptionUIState GetUserInterfaceState(EntityUid uid, BountyHunterRedemptionComponent component, EntityUid actor)
    {
        bool paidToStation = true;
        // Check that the medical redeemer has a valid medical bounty inside
        if (!_container.TryGetContainer(uid, component.Bounty_Container, out var container) ||
            container.ContainedEntities.Count <= 0)
        {
            return new BountyHunterRedemptionUIState(BountyHunterRedemptionStatus.NoBody, 0, paidToStation);
        }
        ...
        {
            return new BountyHunterRedemptionUIState(BountyHunterRedemptionStatus.NoBounty, 0, paidToStation);
        }
        ...
        if (damageable.TotalDamage > bounty.MaximumDamageToRedeem)
        {
            return new BountyHunterRedemptionUIState(BountyHunterRedemptionStatus.Died, 0, paidToStation);
        }

        // Check that the mob is alive.
        if (mobState.CurrentState != Shared.Mobs.MobState.Alive)
        {
            return new BountyHunterRedemptionUIState(BountyHunterRedemptionStatus.Valid, 0, paidToStation);
        }
        ...
        return new BountyHunterRedemptionUIState(BountyHunterRedemptionStatus.Valid, int.Max(bountyPayout, 0), paidToStation);
    }
}
